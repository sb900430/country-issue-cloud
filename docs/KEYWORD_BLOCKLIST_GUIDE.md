...
운영 결과에서 정치인·정당명처럼 경제 이슈 TOP 5에 적합하지 않은 표현을 발견하면 `config/keyword-blocklist.yml`에 국가별 규칙으로 추가한다. 코드 상수를 수정하지 않아도 목록을 점진적으로 확장할 수 있고 모든 변경은 Git 이력으로 남는다.
...
1. 비공개 관리자 Actions artifact의 `selected-articles.json`과 실제 TOP 5를 확인한다.

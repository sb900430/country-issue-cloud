...
- NewsData.io는 설정된 반복 템플릿 매체를 제외하고 일본은 경제 관련 제목 용어 gate를 통과한 기사만 채택한다. GDELT는 반복 HTTP 429로 일시 비활성이며 제한된 단일 요청의 정상 응답과 이용조건 재확인 후에만 다시 켠다.
...
- 세 국가 모두 70건·TOP5를 충족한 날만 live 공개 성공으로 계산한다.
- 70건은 표본량 하한이며, 최소 4건 또는 5%·2매체·서로 다른 관련 기사 집합을 충족한 TOP5가 모두 있어야 한다.
...
- NewsData.ioは設定済み反復template媒体を除外し、日本は経済関連title用語gateを通過した記事だけを採用する。GDELTは反復HTTP 429により一時無効で、制限付き単一requestの正常応答と利用条件を再確認した後だけ再有効化する。
...
- 3か国すべてが70件・TOP5を満たした日だけlive公開成功として数える。
- 70件はsample量下限であり、最低4件または5%・2媒体・異なる関連記事集合を満たすTOP5がすべて必要となる。

...
| 항목 | 현재 상태 |
|---|---|
| 현재 목표 | 기사 표본 보존과 사건 단위 키워드 집계 안정화 |
| 상태 | 구현·한일 문서·전체 검증 완료, 운영 표본 관찰 대기 |
| 기준 브랜치 | `main` |
| 작업 브랜치 | `codex/keyword-pipeline-reliability` |
| 마지막 완료 커밋 | `de16f08e` — 키워드 한국어 표시 추가 |
| 전체 검증 | PASS — Python 172개·Web 14개, backend coverage 90%, Ruff·mypy·Secret·명세 동기화 |
| 다음 작업 | 사용자 승인 시 commit·push·Draft PR 후 다음 예약 실행의 국가별 기사·사건 수 관찰 |

## v1.0 공개 준비 진행 결과

- 2026-08-20 발행처 20%/30건 제한이 기사 삭제로 적용되어 매체 수가 적은 날 표본이 과도하게 줄고, 서로 다른 URL의 재배포 기사가 키워드 빈도를 부풀리는 문제를 수정했다. 같은 URL만 제거하고 나머지는 12시간 내 제목·URL 경로 기반 독립 사건으로 군집화한다. 기사는 국가 상한까지 유지하되 발행처 순위 기여만 가중하고, 키워드 문서 빈도·Jaccard·article ratio는 사건 단위로 계산한다. 단독 일반어와 일반어끼리의 조합을 차단하면서 구체 명사와 결합한 복합어는 후보 근거로 보존하고, 같은 근거에서는 짧은 구체 단어를 우선한다. URL 고유 기사 50건·독립 사건 30건 게이트와 진단 Schema 1.2를 추가했다.

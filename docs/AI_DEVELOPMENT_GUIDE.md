...
2. `git status`로 사용자 변경사항을 확인하고 보존한다.
3. 요구사항과 영향 파일을 확인한다.
4. 테스트 또는 검증 기준을 먼저 정한다.
5. 가장 작은 완결 단위로 구현한다.
6. 관련 테스트 후 `scripts/verify-all.ps1`을 실행한다.
7. 한국어·일본어 명세에 영향을 주면 두 파일을 함께 수정한다.
8. 개발 상태 문서를 갱신한다.
9. 목표 범위를 검증한 후 목표 단위 커밋을 만든다.
...
목표별 작업 브랜치에서 로컬 WIP 커밋을 사용할 수 있다. 목표 완료 시 WIP을 squash해 명세의 목표 커밋 하나로 정리한다. Critical/High 사후 수정만 별도 `fix:` 커밋을 허용한다. 목표 완료 후 상태 문서에 커밋 SHA와 검증 결과를 기록한다.

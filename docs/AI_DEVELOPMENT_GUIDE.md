...
9. WIP 커밋을 squash해 목표 단위 커밋 하나로 정리한다.
10. 목표 브랜치를 push하고 `main` 대상 Draft PR을 만든다.
11. CI와 리뷰가 통과하면 Ready로 전환하고 **Rebase and merge**로 병합한다.
12. 최신 `main`으로 전환해 `scripts/verify-all.ps1`과 가능한 로컬 smoke test를 다시 실행한다.
13. 병합 후 검증이 통과한 경우에만 목표를 완료 처리하고 다음 목표 브랜치를 만든다.
...
- 일일 보고서는 Git 추적 대상이며 현재 목표 브랜치에 보관한다. 일일 보고서만을 위한 별도 커밋은 만들지 않고 목표 최종 커밋과 PR에 포함한다.
- 토요일의 `reviews/` 자동 리뷰 파일은 로컬 전용이므로 일일 보고서와 분리한다.

...
class StaticJsonPublisher:
    ...
    def publish(self) -> list[Path]:
        ...
        dated_sources = sorted(self.published_dir.glob("issues_????-??-??.json"))[-7:]
        ...
        if latest.date not in {result.date for _, result in validated}:
            raise ValueError("latest result has no matching dated result")
        ...
        outputs.append(self.site_data_dir / "latest.json")
...
def public_issue_path(site_data_dir: Path, target_date: date) -> Path:
    return site_data_dir / f"{target_date.isoformat()}.json"

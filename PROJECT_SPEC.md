...
매일 미국·일본·한국 언론사의 경제뉴스를 국가별로 150건 수집하는 것을 목표로 하고 최대 250건까지 독립 수집한다. 제목과 제공된 짧은 요약에서 반복 출현하는 명사·복합명사 후보를 추출하고, 같은 의미의 표현을 하나의 키워드로 묶어 독립 사건 수와 매체 다양성 기준의 국가별 키워드 TOP 5를 계산한다. 사용자는 키워드를 눌러 재배포본을 포함한 해당 키워드의 근거 기사 목록을 확인한다.
...
- 언어별 분석기는 반복 명사와 최대 2개 형태소의 짧은 복합명사를 함께 보존하고, 로컬 다국어 임베딩은 같은 국가 안의 고신뢰 유사 표현만 선택적으로 통합한다. 화면 label은 문장 조각이 아닌 하나의 이슈 개념을 우선한다. 정치인·정당명처럼 경제 이슈에서 제외할 표현은 `config/keyword-blocklist.yml`의 국가별 규칙으로 버전 관리한다.
...
| F-02 | 정제·사건 군집화 | 같은 URL만 제거하고 제목·URL 경로·발행 시각으로 재배포 기사를 동일 사건으로 군집화 |
| F-03 | 키워드 추출 | 영어·일본어·한국어별 반복 명사와 최대 2개 형태소의 짧은 복합명사 추출, 일반어·문장 조각 및 국가별 금지 키워드 제외 |
| F-04 | 키워드 통합 | 로컬 다국어 임베딩으로 국가 내부 동의어·표기 변형을 근거 표현 안에서 제한적으로 통합 |
| F-05 | TOP 5 | 키워드별 독립 사건 수와 매체 다양성 가중치로 결정적 순위 산출 |
...
| F-11 | 날짜 조회 | 최근 7일을 왼쪽 과거·오른쪽 최신 순서로 제공 |
...
| F-18 | 키워드 언어 전환 | 미국·일본 탭에서 키워드를 원문/한국어로 로컬 전환하고 한국 탭에서는 전환 입력을 숨김 |
...
| 국가별 목표 | 중복 제거 후 150개 |
| 국가별 최대량 | 250개 |
| 정상 | URL 고유 기사 50개·독립 사건 30개 이상, 품질 키워드 3~5개 |
| 표본 미달 | URL 고유 기사 49개 이하 또는 독립 사건 29개 이하, 게시하지 않음 |
| 단일 매체 최대 순위 기여 | 선택 기사 수의 20% 또는 30개 중 작은 값에 해당하는 가중치 예산 |
| 매체 편중 경고 | 30% 초과 |
| 심각한 편중 | 50% 초과 |

100건 이상을 권장 수집 목표로 유지하되 국가별 게시 하한은 당분간 URL 고유 기사 50건과 독립 사건 30건으로 정한다. 적법성과 투명성을 우선하며 실제 선택 기사 수를 화면에 표시하고, 하한을 채우기 위해 24시간 범위를 임의로 늘리지 않는다. 50~99건 공개 결과는 표본 변동성이 커질 수 있으므로 국가별 기사·사건 수와 TOP 5 품질을 매일 관찰한다. 매체 편중은 기사를 삭제해 표본량을 줄이지 않고 순위 기여 가중치만 낮춘다.
...
1. 추적 파라미터를 제거한 URL이 같으면 상세 정보가 많은 한 건만 남긴다.
2. 서로 다른 URL은 삭제하지 않고 같은 국가·12시간 이내에서 정규화 제목 일치, 제목 유사도 0.88 이상 또는 충분히 긴 정규화 URL 경로와 제목 문맥이 함께 일치하는지로 동일 사건을 군집화한다.
3. 동일 사건의 재배포 기사들은 키워드 문서 빈도에 한 번만 반영하되 출처·원문 링크는 보존한다.
4. 발행처별 기사는 국가 상한까지 round-robin으로 보존하고, 한 발행처의 순위 기여만 선택 기사 수의 20% 또는 30건 중 작은 값까지 가중한다.

관련 기사 목록은 키워드 근거가 제목·제공 요약에 실제 존재하는 기사만 포함하고, 제목 의미 중심성과 최신성 순으로 재배포 링크를 포함해 최대 20개를 선택한다.

---

## 6. 키워드 분석, LLM과 집계

언어별 분석기는 제목에서 반복 명사와 최대 2개 형태소의 짧은 복합명사를 추출한다. 한국어는 `kiwipiepy`, 일본어는 `SudachiPy` core 사전, 영어는 정규화된 단어·2단어 명사 표현 규칙으로 확정한다. `경제`, `시장`, `정부`, `지원`, `공급`, `기업`처럼 단독으로 이슈를 식별하기 어려운 일반어는 단독 후보에서 제외하되 `주택공급`·`住宅供給`·`housing supply`처럼 구체 명사와 결합한 복합어는 보존한다. 두 구성어가 모두 일반어인 조합은 제외하고, 일반어가 한쪽에 포함된 복합어는 같은 근거의 더 짧은 구체 단어보다 순위 우선권을 갖지 않는다. 한 제목에서 여러 후보를 만들되 화면 label은 하나의 이슈 개념만 나타내며 문장 앞부분을 그대로 후보로 사용하지 않는다.

운영 결과에서 발견한 정치인·정당명 등 명시적 제외 표현은 `config/keyword-blocklist.yml`에서 US·JP·KR별로 관리한다. 규칙은 `exact` 또는 `contains`, 분류, 한·일 제외 사유, 추가일과 활성 상태를 포함한다. 차단은 문서 빈도 계산 전에 적용하며 설정 파일이 없거나 Schema가 잘못되면 기존 정상 배포를 유지하도록 배치를 실패시킨다. 목록 변경은 코드 변경과 동일하게 Git 이력·테스트·PR 검토를 거치며 자세한 절차는 `docs/KEYWORD_BLOCKLIST_GUIDE.md`를 따른다.

동일 사건에서 한 키워드가 여러 기사에 반복되어도 문서 빈도는 1건으로 계산한다. URL 고유 기사 50건·독립 사건 30건 이상의 운영 표본에서 최소 3사건 또는 전체 사건의 2% 중 큰 값과 서로 다른 2개 이상 매체를 충족한 후보만 순위에 포함한다. 날짜·요일·월/분기 표현, 통화 단위, 보도자료 관용어, 출시·실적·주가·이동평균 같은 반복 템플릿 일반어를 제외하고, 임베딩을 사용하는 단독어는 제목 응집도 0.5 미만이면 제외한다. TOP 5 사이의 관련 사건 집합 Jaccard 유사도가 0.5 이상이면 후순위 후보를 버려 서로 다른 이슈를 보장한다. 원시 기사 수만으로 순위를 정하지 않으며 사건별 1회 빈도, 매체 기여 가중치, 제목 응집도, 매체 다양성, 복합어 구체성을 함께 반영한다.

기본 운영의 동의어 통합은 외부 API가 아닌 `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2` 로컬 임베딩을 사용하며 검증한 모델 revision `e8f8c211226b894fcb81acc59f3b34ba3efd5f42`를 고정한다. 같은 국가에서 각 표현이 2개 기사·2개 매체 이상에 나타나고 공백 제외 4자 이상이며 코사인 유사도 0.95 이상일 때만 병합하고 한 클러스터를 최대 3개 표현으로 제한한다. 빈도가 높은 후보를 대표점으로 쓰고 표시명은 실제 기사 후보 중 문서 빈도가 높은 짧은 단어·복합명사로 결정한다. 모델 다운로드 실패나 분석 실패 시 새 결과를 게시하지 않고 기존 정상 Pages를 유지한다.
...
외부 LLM은 기본 운영에서 비활성으로 유지한다. 후속 보정 기능으로 재개하더라도 국가 내부 후보 클러스터의 표시명 보조만 허용하고, 입력 후보에 없는 표현 생성, 국가 간 병합, 순위 결정, 투자 판단은 하지 않는다.
...
```text
document_frequency = 키워드가 1회 이상 등장한 독립 사건 수
publisher_count    = 키워드 관련 고유 매체 수
article_ratio      = document_frequency / 국가 독립 사건 수(필드명은 Schema 2.0 호환 유지)
keyword_score      = 매체 가중 사건 빈도 × 제목 의미 응집도 × 매체 다양성 × 복합어 구체성, 최신 시각·keyword_id 순 동률 해소
```

`success`: 세 국가가 각각 URL 고유 기사 50개·독립 사건 30개 이상, 키워드 처리 성공률 80% 이상, 품질 키워드 3~5개를 충족.
...
기사 수 50건과 독립 사건 30건은 표본량 게이트일 뿐 품질 통과를 의미하지 않는다. 위 빈도·매체·일반어·중복 이슈 기준을 모두 만족하는 키워드를 최소 3개 만들 수 없으면 그 국가만 실패한다. 품질을 낮춰 5개를 강제로 채우지 않는다.
...
`label`은 집계·순위·근거에 사용하는 원문이며 `label_ko`는 표시 전용 한국어 보조명이다. 새 결과는 `label_ko`를 항상 만들고, 번역 사전에 없는 표현은 추측하지 않고 원문을 그대로 저장한다. 이전 7일 Schema 2.0 결과와의 호환을 위해 client는 누락된 `label_ko`도 허용한다. Pages `preserve` 배포는 외부 뉴스 API를 다시 호출하지 않고 복원한 이전 결과에 현재 사전을 적용해 공개 JSON의 `label_ko`를 보충한다. 번역은 `config/keyword-translations.yml`의 정규화된 완전 일치 사전으로 관리하며 외부 번역 API와 유료 LLM을 호출하지 않는다.
...
홈 화면은 앱 제목/기준일, 국가 탭, 최근 7일, `오늘의 이슈 TOP 5` 시각화, 마지막 업데이트 시각과 새로고침 순이다. 국가 탭은 같은 날짜 응답을 사용해 추가 요청 없이 전환하고 날짜 변경 때만 요청한다. 최근 7일 탭은 왼쪽이 가장 과거이고 오른쪽이 최신이며, 좁은 화면에서는 선택한 날짜가 보이도록 가로 스크롤한다.
...
| 키워드 언어 | 미국·일본 탭은 기본 한국어와 `원문 / 한국어` 세그먼트를 제공하고 한국 탭은 세그먼트를 숨김 |
...
키워드 언어 전환도 데이터 재요청·재집계 없이 타일, 클라우드, 상세 제목에 동시에 적용한다. 미국·일본 탭 사이를 이동해도 현재 선택 모드를 유지하고 다시 렌더링한다. 한국 탭에서는 전환 입력을 숨기고 한국어 원문만 표시한다. 관련 기사 제목과 링크는 원문을 유지한다. 미국·일본의 번역 사전 미등록 표현은 원문으로 표시하며, 잘못된 번역을 임의 생성하지 않는다.
...
6. 로컬 다국어 임베딩 동의어 통합과 선택적 후속 LLM 표시명 보정
7. 키워드 근거 기사 검증
8. 국가별 TOP 5 집계
9. 품질 리뷰
10. 게시 조건 판정
...
순서 5는 무료 소스 보강 전에 국가·소스별 원본 수신 건수, 소스별 채택 매체 분포, URL 중복 제거 후 건수, 독립 사건 수, 최종 선택 기사 수와 매체 가중 유효량을 계측한다. 진단 Schema 1.2에는 기사 제목·URL·ID·Secret 없이 집계값만 기록한다. NAVER 일 300회·월 9,000회와 유료 자동 전환 금지는 유지하며, 소스·query 변경은 허용 domain과 이용조건을 확인한 항목만 적용한다.
...
2026-08-13 운영 결정으로 무료 소스의 일별 변동을 관찰하는 동안 국가별 게시 하한을 50건으로 임시 조정한다. 권장 수집 100건 이상, 목표 150건, 최대 250건, 매체별 20%/30건 제한과 TOP 5 품질 게이트는 유지한다. 매일 12:00 보충 schedule 이후 국가별 최종 선택 기사 수와 게시 성공 여부를 채팅 보고 대상으로 삼고, 50~99건 결과의 키워드 품질을 관찰해 하한 복원 여부를 결정한다.
...
2026-08-20 수집량 손실과 재배포 기사 과대 집계를 함께 해결하기 위해 매체 20%/30건 기준을 기사 삭제에서 순위 기여 가중치로 변경한다. URL이 다른 기사는 보존하고 12시간 내 정규화 제목·URL 경로 유사성으로 사건 군집을 부여하며, 키워드 빈도는 사건별 한 번만 세고 관련 링크는 유지한다. 단독 일반어와 두 일반어 조합은 차단하되 구체 명사와 결합한 복합어는 유지하고, 진단 Schema 1.2에 독립 사건 수와 가중 유효량을 기록한다. 게시 표본은 URL 고유 기사 50건과 독립 사건 30건을 모두 요구한다.
...
> 국가별 이슈 클라우드는 미국·일본·한국의 경제뉴스를 국가별로 독립 수집하고, 로컬 다국어 임베딩으로 각 국가 내부의 유사한 기사 표현을 이슈 단위로 묶은 뒤, 고유 기사 수와 매체 다양성에 따라 국가별 TOP 5를 URL로 보여주는 반응형 웹 서비스다. 결과에는 실제 출처와 표본 수를 제공하며, 배치 실패·캐시 복구·외부 API 비용과 같은 운영 문제를 명시적으로 처리한다. Android 앱은 공개 웹 안정화 후 선택적으로 재개할 수 있도록 API 계약과 설계 기록을 보존한다.

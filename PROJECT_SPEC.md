...
| 상태 저장 | 최초 실행은 타일형, 이후에는 DataStore에 마지막 선택을 저장하고 재실행 시 복원 |
| 공통 제목 | `오늘의 이슈 TOP 5` |
...
보기 전환은 데이터 재요청이나 재집계를 발생시키지 않고 동일한 ViewModel 상태를 다른 Compose 컴포넌트로 렌더링한다. 국가·날짜·로딩·오류 상태와 스크롤 위치는 전환 과정에서 유지한다.
...
- 통합 검증과 CI, 리뷰 통과 후 Ready로 전환하고 **Rebase and merge**로 병합해 검증된 목표 커밋 제목과 선형 이력을 유지한다.

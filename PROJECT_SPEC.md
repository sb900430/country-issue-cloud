...
| 국가별 최대량 | 250개 |
| 정상 | 70개 이상, 키워드 5개 |
| 부분 성공 | 50~69개, 게시하지 않음 |
| 단일 매체 최대 반영 | 수집 결과의 20% 또는 30개 중 작은 값 |
| 매체 편중 경고 | 30% 초과 |
| 심각한 편중 | 50% 초과 |

100건 이상을 권장 수집 목표로 유지하되 국가별 게시 하한은 70건으로 정한다. 적법성과 투명성을 우선하며 실제 수집·중복 제거 후 기사 수를 화면에 표시하고, 하한을 채우기 위해 24시간 범위를 임의로 늘리지 않는다.
...
`success`: 세 국가가 각각 기사 70개 이상, 키워드 처리 성공률 80% 이상, 키워드 5개를 모두 충족.
